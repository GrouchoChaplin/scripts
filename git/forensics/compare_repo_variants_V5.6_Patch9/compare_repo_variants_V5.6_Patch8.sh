#!/usr/bin/env bash
#
# compare_repo_variants_V5.6_Patch8.sh
#
# Deep forensic comparison of multiple backups/copies of the same Git repo.
#
# Features:
#   - Find all repo copies whose directory name *contains* a given substring
#   - STANDARD mode:
...
#   - DEEP COMPARE mode (optional flag):
#       * For baseline (most active) vs every other repo:
#           - Show which files differ / added / removed
#           - Write unified diffs to files
#   - HTML REPORT (optional flag):
#       * Forensic table as HTML
#       * Deep compare summaries as <details> blocks with diff output
#   - LOGGING (optional flag):
...
deep_compare_pair() {
    ...
    # Write full unified diff (may be large)
    diff -ru --exclude='.git' "$base" "$other" >"$diff_file" 2>&1 || true

    # Also do a quick classification summary (added/removed/changed)
    local only_base=0
    local only_other=0
    local differ=0
...
deep_compare_from_scanfile() {
    ...
    # baseline: first line of sorted (highest activity epoch)
    local first_line
    ...
    # For every other line in sorted file
    while IFS=$'\t' read -r activity_epoch repo branch last_epoch last_human dirty ahead behind \
                                 staged unstaged untracked latest_epoch latest_human latest_path; do
...
run_mode_forensic() {
    ...
    if [[ "$HTML_ENABLED" -eq 1 ]]; then
        ...
        # Need to re-iterate sorted file to feed HTML (since we consumed it above)
        html_append_forensic_table "$sorted"
    fi

    # Deep compare across variants
...
Options:
  ...
  --deep-compare        After forensic table, deeply diff baseline vs each other repo
  --html                Produce an HTML forensic report
  --log                 Save all output to a timestamped log file
...
Examples:
  ...
  Forensic + deep compare + HTML + log:
    $0 --root-folder /run/media/.../2025-10-10 \\
       --repo-name jsigconversiontools \\
       --mode forensic --deep-compare --html --log \\
       --debug-find --debug-scan --debug-temp
EOF
}
...
if [[ "$LOG_ENABLED" -eq 1 ]]; then
    ...
    # tee both stdout and stderr into the log
