...
# Flutter Source Version Finder v1.7
#
...
# - v1.7: latest-per-file analysis for .dart files with group-by modes
#
set -euo pipefail
...
# v1.7: per-file analysis
LATEST_PER_FILE=0           # when 1, compute latest per .dart file
...
Group-by modes (used with --latest-per-file):

  --group-by basename
        Group files only by their filename.
        Example: all of these are grouped together:
            /A/lib/screens/home/home_screen.dart
            /B/lib/models/home_screen.dart
        Useful for finding the newest version of a file name across all projects,
        even if the directory structure differs.

  --group-by relpath
        Group files by their relative path under lib/.
        Example: these are grouped together:
            /A/lib/screens/home/home_screen.dart
            /B/lib/screens/home/home_screen.dart
        But this file is NOT grouped with them:
            /A/lib/models/home_screen.dart
        Useful when you want to compare structurally equivalent files
        across multiple project copies.

  --group-by fullpath
        No grouping. Every file is treated as distinct.
        Useful for debugging or auditing all source files individually.
...
For latest-per-file Dart analysis across backup trees:
  find_latest_flutter_source.sh \\
    --paths "/run/media/peddycoartte/MasterBackup/ProjectWorkingCopyBackups/ir_imagery_tools*" \\
    --latest-per-file --group-by basename
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --group-by)
      if [[ $# -lt 2 ]]; then
        echo "Error: --group-by requires a MODE (basename|relpath|fullpath)" >&2
        exit 1
      fi
      case "$2" in
        basename|relpath|fullpath)
          ...
          ;;
        *)
          echo "Invalid value for --group-by: $2 (expected basename|relpath|fullpath)" >&2
          exit 1
          ;;
      esac
      shift 2 ;;
...
# v1.7: per-file latest .dart analysis
LATEST_PER_FILE_RAW="$TMPDIR/latest_per_file_raw.tsv"
LATEST_PER_FILE_TSV="$TMPDIR/latest_per_file.tsv"
...
########################################
# v1.7: Latest-per-file analysis for .dart files
########################################
if [[ $LATEST_PER_FILE -eq 1 ]]; then
  echo ""
  echo "$(bold "🧩 Latest per Dart file (group-by: $GROUP_BY_MODE)"))"
  echo "-----------------------------------------------------------------------------"

  # Build raw key → best (.dart only)
  awk -F'\t' -v mode="$GROUP_BY_MODE" '
    function get_basename(path,   n, parts) {
      n = split(path, parts, "/");
      return parts[n];
    }
    function get_relpath_under_lib(path,   idx, rel, n, parts) {
      idx = index(path, "/lib/");
      if (idx == 0) {
        # No /lib/, fall back to basename
        return get_basename(path);
      }
      # Skip "/lib/" (5 chars) and everything before it
      rel = substr(path, idx + 5);
      if (rel == "") {
        # Just in case, fall back to basename
        return get_basename(path);
      }
      return rel;
    }
    {
      ...
      # Only consider Dart files for per-file analysis
      if (path !~ /\.dart$/) {
        next;
      }
      ...
      if (mode == "basename") {
        key = get_basename(path);
      } else if (mode == "relpath") {
        key = get_relpath_under_lib(path);
      } else if (mode == "fullpath") {
        key = path;
      } else {
        # Fallback: behave like basename
        key = get_basename(path);
      }

      # Track best (latest) by epoch
    ...
    END {
      for (k in best_epoch) {
        printf "%s\t%s\t%s\t%s\n", best_epoch[k], best_ts[k], k, best_path[k];
      }
    }
  ' "$RAW_FILES" > "$LATEST_PER_FILE_RAW"

  if [[ ! -s "$LATEST_PER_FILE_RAW" ]]; then
    echo "No .dart files found for latest-per-file analysis."
  else
    sort -r -n -k1,1 "$LATEST_PER_FILE_RAW" > "$LATEST_PER_FILE_TSV"

    unique_count=$(wc -l < "$LATEST_PER_FILE_TSV")
    echo "Unique Dart file groups: $unique_count"
    echo ""
    printf "%-22s %-40s %s\n" "Latest Timestamp" "Group Key" "Path"

    while IFS=$'\t' read -r epoch ts key path; do
      printf "%-22s %-40s %s\n" "$ts" "$key" "$path"
    done < "$LATEST_PER_FILE_TSV"
  fi
fi
...
EOF
    ...
    # Optional: include a small note if latest-per-file was requested
    if [[ $LATEST_PER_FILE -eq 1 && -s "$LATEST_PER_FILE_TSV" ]]; then
      echo "  </table>"
      echo "  <h3>Latest per Dart File (group-by: $GROUP_BY_MODE)</h3>"
      cat <<'EOF'
  <table id="latestPerFileTable">
    <tr>
      <th onclick="sortTable('latestPerFileTable', 0)">Timestamp</th>
      <th onclick="sortTable('latestPerFileTable', 1)">Group Key</th>
      <th onclick="sortTable('latestPerFileTable', 2)">Path</th>
    </tr>
EOF
      while IFS=$'\t' read -r epoch ts key path; do
        printf '    <tr><td class="mono">%s</td><td class="mono">%s</td><td class="mono">%s</td></tr>\n' \
