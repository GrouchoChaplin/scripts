...
Options:
  ...
  --diff-pattern <glob>  Restrict diff to files matching patterns (can repeat)
                         Pattern style:
                           - Starts with '/' → full-path glob
                           - Otherwise       → matches anywhere in the path
...
# ────────────────────────────────────────────────────────────────
# HELPER: pattern match for diff paths (style C)
# ────────────────────────────────────────────────────────────────
matches_diff_patterns() {
    ...
    for pat in "${DIFF_PATTERNS[@]}"; do
        local eff
        if [[ "$pat" == /* ]]; then
            # Full-path glob (user-specified)
            eff="$pat"
        else
            # "Anywhere in path" match (prefix wildcard)
            eff="*${pat}"
        fi
        case "$path" in
            $eff) return 0;;
    ...
    for dir in "${REPO_DIRS[@]}"; do
        ...
        case "$DIFF_LEVEL" in
            summary)
                ...
                if ((only_in_best == 0 && only_in_other == 0 && differ == 0)); then
                    echo "   (no differences matching given patterns)"
                else
