#!/usr/bin/env bash
#
# compare_repo_variants_V5.6_Patch3.sh
...
compare_repo_variants_V5.6_Patch3.sh
────────────────────────────────────
Compare multiple nightly backups of a Git repo and determine:
  • latest commit
  • most recent modified file
  • untracked/unstaged/staged changes
  • which backup is likely the "last active" snapshot
  • per-file diff, summary diff, full diff (in standard mode)
  • complete forensic reconstruction (forensic mode)
...
Examples:
  Standard comparison:
    $0 --root-folder /backup/Nightly/2025-10-10/projects \
       --repo-name jsigconversiontools \
       --mode standard

  Forensic investigation:
    $0 --root-folder /backup/.../jctcs \
       --repo-name jsigconversiontools \
       --mode forensic
