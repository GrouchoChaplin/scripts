...
if [[ "$DO_LOG" == "yes" ]]; then
    ...
    LOGFILE+=".log"
    # Announce to stderr (before redirect)
                ...
                while IFS= read -r line; do
                    case "$line" in
                        "Only in "*)
                            ...
                            ;;
                        "Files "*)
                            # Lines: Files <path1> and <path2> differ
                            if [[ "$line" == *" differ" ]]; then
                                ((differ++))
                            fi
                            ;;
...
echo "   (debug) Raw diff output:" >&2
diff -qr --exclude='.git' "$BEST_PATH" "$dir" >&2
echo "   (end debug)" >&2
